Student Achievement Tacker 
By: Ryan Potter, Ben, Mohib, Dorian 
 
CHANGES FROM PHASE II SUBMISSION: 
Please see the submitted Phase III document's appendix. 
...
Many of the Objects we created in JavaScript for returning database query are not transferring out of  
their file. We use the callback method that JavaScript has for the asynchronous computing that is inherent 
...
what the connectivity between objects would be if we had managed to achieve the connectivity that we wanted. 
 
Additionally our testing has issues. We were unable to test the functionality in server.js because it is a router and acts sort of like our "main" function in Java. Because of the previously mentioned workarounds it was hard to test our queries. But every function that we did call from server.js has a test. 
